...
def lidar_cloud(source: str, limit: int = 6000) -> dict:
    """Point cloud passthrough for the 3D viewer. First queries web_dashboard
    and standalone sensor bridges (hesai_bridge :5003 / webrtc_bridge :5001),
    falling back to core."""
    pts = []
    try:
        if source == "go2":
            for url in ("http://127.0.0.1:5002/lidar_proxy", "http://127.0.0.1:5001/lidar", f"{CORE}/lidar/go2"):
                try:
            ...
            for url in (f"http://127.0.0.1:5002/lidar_hesai_proxy?limit={limit}", f"http://127.0.0.1:5003/lidar?limit={limit}", f"{CORE}/lidar/hesai"):
                try:
                    r = _sess().get(url, timeout=1.5)
                    if r.status_code == 200:
                        data = r.json()
